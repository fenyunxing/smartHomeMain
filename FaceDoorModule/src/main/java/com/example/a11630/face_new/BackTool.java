...
public class BackTool {
    ...
    public static void SendHttpInsert(final String ID, final String name_or_date, final int type) {
        String uu = null;
        if (type == 1) {             //insert
            uu = "http://sumtudou.cn/Database/operate?opt={\"ClockInActivity\":\"insert\",\"id\":\"" + ID
                    + "\",\"name\":\"" + name_or_date + "\"}";
        } else if (type == 2) {      //in_clock
            uu = "http://sumtudou.cn/Database/operate?opt={\"ClockInActivity\":\"in_clock\",\"id\":\"" + ID
                    + "\",\"date\":\"" + name_or_date + "\"}";
        }
    ...
    public static String SendHttpDelete_orQueryAllface(final String ID, final int type) {  //懒得改了，直接重写了
        ...
        StringBuffer response = new StringBuffer();
        if (type == 1) {
            uu = "http://sumtudou.cn/Database/operate?opt={\"ClockInActivity\":\"DeleteActivity\",\"id\":\"" + ID
                    + "\"}";
        } else if (type == 2) {
            uu = "http://sumtudou.cn/Database/operate?opt={\"ClockInActivity\":\"query_face\"}";
        }
